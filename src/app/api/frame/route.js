...
export async function POST(req) {
  ...
  const quotesResponse = await fetch(`http://${req.headers.get("host")}/api/quote`, { method: "GET" });
  const { quotes } = await quotesResponse.json();

  // Manage current quote index
  let currentIndex = untrustedData?.state?.currentIndex || 0;
  if (!quotes.length) currentIndex = 0;
  else if (buttonIndex === 1) currentIndex = (currentIndex - 1 + quotes.length) % quotes.length; // Previous
  ...
  if (buttonIndex === 3 && inputText) {
    ...
    const updatedQuotesResponse = await fetch(`http://${req.headers.get("host")}/api/quote`, { method: "GET" });
      ...
      `<!DOCTYPE html>
       <html>
         <head>
           ...
           <meta property="fc:frame:post_url" content="${req.url}" />
           <meta property="fc:frame:state" content="${JSON.stringify({ currentIndex: updatedQuotes.quotes.length - 1, fid })}" />
         </head>
       </html>`,
      { status: 200, headers: { "Content-Type": "text/html" } }
    );
    ...
    `<!DOCTYPE html>
     <html>
       <head>
         ...
         <meta property="fc:frame:input:text" content="Enter your quote" />
         <meta property="fc:frame:post_url" content="${req.url}" />
         <meta property="fc:frame:post_url:4" content="https://${req.headers.get("host")}/frame-ui?fid=${fid}" />
         <meta property="fc:frame:state" content="${JSON.stringify({ currentIndex, fid })}" />
       </head>
       <body></body>
     </html>`,
    { status: 200, headers: { "Content-Type": "text/html" } }
